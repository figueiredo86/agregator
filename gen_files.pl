...
=cut
 ...
 while (@user_data=$get_user->fetchrow_array) {
  open (my $sip_exten, '>>', '/root/projetos/perl/agregator/sip_extensions.conf') or die "arquivo /projetos/perl/yealink_provisioner/sip_extensions.conf não existe ou sem permissão";
...
sub create_prov_file {
 system("touch","/root/projetos/perl/agregator/$exten_mac.cfg");
 open (my $prov_file,">","/root/projetos/perl/agregator/$exten_mac.cfg") or die "arquivo /root/projetos/perl/yealink_provisioner/'$exten_mac'.cfg não existe ou sem permissão";
